  ...
  namespace beethowen_receiver
  {
    ...
    void BeethowenReceiverHub::beethowen_on_command_(const uint8_t command, const uint8_t *buffer, const int size)
    {
      ...
      case beethowen_base::BeethowenCommand_FindServerRequest:
      {
        ...
        ESP_LOGD(TAG, "---- Allowed devices ----");

        for (const auto &it : get_devices_debug())
        {
          auto addr = it.first;
          auto dev = it.second;
          ...
          ESP_LOGD(TAG,
                  ...
                  bthome_base::addr64_to_str(addr).c_str(),
                  addr);
        }
      ...
      case beethowen_base::BeethowenCommand_Data:
      {
        ...
        auto packet_id = parse_message_bthome_(client_mac_a64, payload_data, payload_len, bthome_base::BTProtoVersion_BTHomeV2);

        // send ack
        if (this->send_data_ack_ && packet_id.has_value())
        {
          uint8_t *client_mac = beethowen_base::sender;
          beethowen_base::send_command_data_ack(client_mac, local_passkey_, (uint8_t)packet_id.value());
        }
      }
      break;
